...
- **群聊分析**：`群分析` 命令调用 LLM 生成报告——热门话题、高光记录（高光对话 + 金句）、活跃榜
...
报告里的「高光记录」板块收两类内容，由两次独立的 LLM 抽取产生，判定标准互不干扰：

- **🧊 高光对话**：连续的多轮对话，须同时满足「有学术要素」（某学科的术语、定理、模型、论文或研究范式，不限理工科）和「是冷幽默」（笑点来自一本正经的推演与反差，而非感叹号、颜文字、网络热梗）。两条缺一不可。
- **💬 金句**：单句就能成立的精彩发言——观点新颖、表达生动或有反差感，跳过热词堆砌和复读。

两类都是宁缺毋滥，找不到就返回空列表；两类都为空时整个板块不渲染。`maxGoldenQuotes` 和 `maxHighlightDialogues` 分别控制两类的条数上限，任一设为 0 即关闭该类，`maxHighlightLines` 控制单段对话最多保留几轮。

模型返回后还会再过一道规整：金句丢掉没有原文的；对话丢掉空轮次、按 `maxHighlightLines` 截断，并要求剩下的至少两轮、至少两人——一个人自问自答不算对话，会被整段丢弃。条目的 `kind` 由插件在这一步打上，不依赖模型返回。
