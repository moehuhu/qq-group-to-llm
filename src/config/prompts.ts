...
4. 相关聊天记录：每个话题附上最能支撑它的若干条原话（直接从「群聊记录」里照抄，不要改写、不要编造、不要转述）。这些记录只用于核对该话题确实讨论过、以及参与者确实是这些人，帮助你避免张冠李戴——它们不会展示在结果中，所以不用考虑排版或长度，按时间顺序照抄即可；找不到足够支撑的话，宁可少给也不要凑
...
- 每条发言的行首都有 <msgid:xxx> 锚点，**逐条照抄对应的锚点原文**，不要改写、不要编造、不要自拟原文；
  发言以「[引用 昵称] 原话」开头的，表示它回复的是那句话，照抄时连这一行一起保留
