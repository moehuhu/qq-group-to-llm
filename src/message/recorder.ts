...
function quotePreview(session: Session, config: Config): string {
  ...
  const flat = decodePlatformMarkup(serializeNodes(nodes, config, true))
    .replace(/\s+/g, ' ')
    .trim()
...
function buildRecord(session: Session, config: Config): MessageRecord {
  ...
  const body = elements.length
    ? decodePlatformMarkup(serializeNodes(elements, config))
    : decodePlatformMarkup(session.content)
