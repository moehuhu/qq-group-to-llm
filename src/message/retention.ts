...
import { cleanupMediaFiles } from '../media'
import { logger } from '../logger'
import { createTimeFormatter } from '../time'

const CLEAN_INTERVAL = 6 * 60 * 60 * 1000 // 每 6 小时执行一次

/** 注册定时任务，删除超出保留期限的过期消息与图片缓存文件 */
export function applyRetentionCleanup(ctx: Context, config: Config) {
  const log = logger(ctx)

  if (config.retentionDays <= 0 && config.mediaRetentionDays <= 0 && config.mediaCacheMaxEntries <= 0) {
    log.info('消息与图片缓存均永久保留，未启用定期清理')
    return
  }
  ...
  log.info(`定期清理已启用：消息保留 ${config.retentionDays > 0 ? `${config.retentionDays} 天` : '永久'}，` +
    ...
    `图片缓存最多 ${config.mediaCacheMaxEntries > 0 ? `${config.mediaCacheMaxEntries} 条` : '不限'}，` +
  ...
  ctx.setInterval(async () => {
    ...
    try {
      ...
      if (config.mediaRetentionDays > 0 || config.mediaCacheMaxEntries > 0) {
        const count = await cleanupMediaFiles(ctx, config)
        removed.push(`图片缓存 ${count} 条`)
      }
